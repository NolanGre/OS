...
ExecStatusType add_calculator(PGconn* conn) {
    ...
    res = PQexec(conn, "LOCK TABLE calculator IN EXCLUSIVE MODE");
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
    ...
    res = PQexec(conn, "INSERT INTO calculator VALUES (2, 'Sverdan', 2005)");
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
    ...
    printf("Adding calculator is successful!\n");
